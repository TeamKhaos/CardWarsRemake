...
func _ready() -> void:
	...
	self.position = Vector2(tamano_ventana.x * 0.78, tamano_ventana.y * 0.18)
...
func tomar_carta():
	if primerclick == false:
		for i in range(5):
			...
			# MODIFICACIÓN CLAVE: Mover la carta al instanciarla.
			# Suma a X para ir a la derecha, resta a Y para ir hacia arriba.
