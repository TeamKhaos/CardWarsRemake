...
func _ready() -> void:
	...
	# Repartir cartas automáticamente al inicio
	call_deferred("tomar_carta_inicial")
...
func tomar_carta_inicial():
	if primerclick == false:
		for i in range(5):
			...
			nueva_carta.aplicar_brillo_elemental(datos_carta["tipo"])
		...
		# Iniciar turno en el GameManager tras repartir
		var gm = get_tree().root.find_child("Game_Manager", true, false)
		if gm: gm.iniciar_turno_jugador()

# --- FUNCIONES DEL MAZO ---
func tomar_carta():
	# Esta función se mantiene para cuando el jugador roba una carta extra
	pass
